import { FileText, Image, Music, Video, ShoppingBag } from "lucide-react";
...
export function AttachmentMenu({ panelRef, onPick, disabled, onCatalog }: Props) {
  ...
  return (
    <div ref={panelRef} className="absolute bottom-12 left-0 z-20 w-52 overflow-hidden rounded-[12px] border border-slate-200 bg-white shadow-xl">
      {onCatalog && <button type="button" className="flex w-full items-center gap-2 px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50" onClick={onCatalog}>
