...
function CatalogSettings() {
  ...
  return <Panel title="WhatsApp catalog" action={<Button size="sm" variant="outline" onClick={query.reload}>Refresh status</Button>}><QueryState {...query} /><ErrorNotice message={action.error} />
    ...
    </> : query.data && <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void action.run("/catalog", { catalogId, confirmDedicatedCatalog: dedicated }, query.reload, "PUT"); }}><p className="text-sm">Connect a dedicated, empty catalog accessible to the connected Meta user. AIWizChat will ask Meta to link it to this WhatsApp account and verify the link before saving it.</p>
      {manage && <><p className="text-xs text-slate-500">Already linked catalogs can be selected below. For an unlinked catalog, paste its Catalog ID. The catalog and WhatsApp account must belong to the same Meta Business Portfolio.</p><Button type="button" variant="outline" size="sm" onClick={() => setListing(true)}>Find already linked catalogs</Button>{listing && <><QueryState {...catalogs} empty={!catalogs.data?.catalogs.length} />{catalogs.data?.catalogs.map((c) => <Button key={c.id} type="button" variant="ghost" onClick={() => setCatalogId(c.id)}>{c.name} · {c.id}</Button>)}{catalogs.data?.cursor && <Button type="button" variant="outline" size="sm" onClick={() => setCursor(catalogs.data!.cursor!)}>More catalogs</Button>}</>}
