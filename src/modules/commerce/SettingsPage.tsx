...
function CatalogSettings() {
  ...
  return <Panel title="WhatsApp catalog" action={<Button size="sm" variant="outline" onClick={query.reload}>Refresh status</Button>}><QueryState {...query} /><ErrorNotice message={action.error} />
    ...
    </> : query.data && <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void action.run("/catalog", { catalogId, confirmDedicatedCatalog: dedicated }, query.reload, "PUT"); }}><p className="text-sm">Connect a dedicated, empty catalog linked to your current WhatsApp Business account.</p>
