...
import { Cloud, FileSpreadsheet, Megaphone, RefreshCcw, Sparkles, Timer } from "lucide-react";
import { broadcastCampaigns, csvCampaigns } from "./campaigns-preview/data";
import { CampaignCard, EmptyState, SkeletonCampaignCard } from "./campaigns-preview/ui";
import { useApiCampaigns } from "./campaigns-preview/useApiCampaigns";
import type { CampaignSource } from "./campaigns-preview/types";
...
export function CampaignsPreview({ inView }: { inView: boolean }) {
  ...
  const { apiLoading, apiError, apiCampaigns, runApiLoad } = useApiCampaigns(active, apiUrl);
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-ink-900/55">
              {active === "broadcast"
                ? "Broadcast to segments using templates"
                : active === "csv"
                  ? "Upload CSV and map template fields"
                  : "Sync campaigns dynamically via API"}
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-1 rounded-2xl border border-ink-900/10 bg-white p-1">
            {[
              ...
              <button
                key={t.key}
                type="button"
                onClick={() => setActive(t.key as CampaignSource)}
                className={[
        ...
        {active === "csv" ? (
          <div className="mt-4 rounded-2xl border border-ink-900/10 bg-white p-4">
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
              {[{ k: "phone", v: "+91 98xxxxxxx" }, { k: "name", v: "Aisha" }, { k: "orderId", v: "OD-18421" }, { k: "coupon", v: "MAY10" }].map((r) => (
        ...
        {active === "broadcast" ? (
          <div className="mt-4 rounded-2xl border border-ink-900/10 bg-white p-4">
            ...
            <div className="mt-4 grid md:grid-cols-3 gap-3">
              {[{ label: "Template", value: "Promo + Coupon" }, { label: "Personalize", value: "{name}, {coupon}" }, { label: "CTA", value: "Shop now -> short link" }].map((x) => (
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-900/55">Dynamic data: events to campaigns to delivery</p>
                </div>
              </div>
              <button type="button" onClick={runApiLoad} className="inline-flex items-center gap-2 rounded-xl border border-ink-900/10 bg-slate-50 px-3 py-2 text-xs font-semibold text-ink-900 hover:border-brand-300/40 hover:bg-brand-50 transition-colors">
                <RefreshCcw className="h-4 w-4" />
                Refresh
              </button>
            </div>
            <div className="mt-4 grid md:grid-cols-3 gap-3">
              {[{ label: "Endpoint", value: apiUrl ? "Connected" : "Demo mode" }, { label: "Mode", value: "Pull JSON + normalize" }, { label: "Binding", value: apiLoading ? "Skeleton to grid" : "Grid to metrics" }].map((x) => (
                <div key={x.label} className="rounded-xl border border-ink-900/10 bg-slate-50 p-3">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-ink-900/45">{x.label}</p>
                  <p className="mt-1 text-xs font-semibold text-ink-900">{x.value}</p>
                </div>
