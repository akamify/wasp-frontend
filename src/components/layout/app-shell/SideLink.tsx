...
export function SideLink({ to, label, kicker, icon: Icon, isCollapsed }: any) {
  ...
  return (
    ...
    >
      ...
      {!isCollapsed ? <div className="flex-1 min-w-0"><div className="text-sm font-semibold leading-tight break-words whitespace-normal">{label}</div><div className={cn("mt-0.5 text-[10px] uppercase tracking-wider opacity-60 font-medium leading-tight whitespace-normal break-words", isCollapsed ? "hidden" : "block")}>{kicker}</div></div> : null}
