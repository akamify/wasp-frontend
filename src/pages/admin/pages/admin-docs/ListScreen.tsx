...
export function ListScreen(props: any) {
  ...
  return (
    <div className="flex flex-col p-6">
      <AdminToolbar title="Docs" subtitle="Manage docs content, preview, publish state, and docs branding from one place." query={query} setQuery={setQuery} onRefresh={load} isSyncing={loading} right={<div className="flex items-center gap-2"><Button variant="outline" onClick={() => navigate(docsBasePath.replace("/docs", "/docs-feedbacks"))}>Feedbacks</Button><Button variant="outline" onClick={() => setBrandModal(true)}>Brand & Logo</Button><Button onClick={() => navigate(`${docsBasePath}/create`)} disabled={!canCreate}>Create Doc</Button></div>} />
