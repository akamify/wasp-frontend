...
export function EditorScreen(props: any) {
  const { navigate, saveDoc, saving, editing, canCreate, canEdit, setEditing, slugify, TOOLBAR, setActiveTool, addedBlocks, syncBlocks, liveContent } = props;
  return (
        ...
        <div className="grid flex-1 grid-cols-1 gap-4 overflow-hidden lg:grid-cols-2">
          <div className="overflow-y-auto rounded-[5px] border border-slate-200 bg-white p-4">
            <div className="space-y-3">
              ...
              <div className="rounded-[5px] border border-slate-200 bg-slate-50 p-3"><div className="mb-2 text-xs font-black uppercase tracking-widest text-slate-500">Add Blocks</div><div className="flex flex-wrap gap-2">{TOOLBAR.map((b: any) => <Button key={b.label} type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => setActiveTool(b)} disabled={(!editing?.id && !canCreate) || (!!editing?.id && !canEdit)}>{b.label}</Button>)}</div></div>
              <div className="rounded-[5px] border border-slate-200 bg-white p-3"><div className="mb-2 text-xs font-black uppercase tracking-widest text-slate-500">Blocks</div><div className="space-y-2">{addedBlocks.map((b: any, idx: number) => <div key={b.id} className="flex items-center justify-between rounded-[5px] border border-slate-200 px-3 py-2 text-xs"><span className="font-black text-slate-900">{idx + 1}. {b.label}</span><Button type="button" variant="ghost" className="h-7 px-2" onClick={() => syncBlocks(addedBlocks.filter((x: any) => x.id !== b.id))}><X className="h-3.5 w-3.5" /></Button></div>)}</div></div>
            </div>
          </div>
