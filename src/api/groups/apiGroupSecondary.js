export function buildApiGroupSecondary(api, unwrap, API_BASE_URL) {
  return {
    ...
    contacts: {
      ...
      attributes: (params) => api.get("/contacts/attributes", { params }).then(unwrap),
      attribute: (key) => api.get(`/contacts/attributes/${encodeURIComponent(key)}`).then(unwrap),
      createAttribute: (payload) => api.post("/contacts/attributes", payload).then(unwrap),
      updateAttribute: (key, payload) => api.patch(`/contacts/attributes/${encodeURIComponent(key)}`, payload).then(unwrap),
      archiveAttribute: (key) => api.delete(`/contacts/attributes/${encodeURIComponent(key)}`).then(unwrap),
      ...
      removeAttributeValue: (id, key) => api.delete(`/contacts/${id}/attributes/${encodeURIComponent(key)}`).then(unwrap),
